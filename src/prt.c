...
char *update_styles(char *format, int clean){
	// increase the format size and use 'src'
	char *src = malloc(MAXSIZ * sizeof(char));
	strcpy(src, format);
...
void draw_box(int x, int y, int widht, int height, char *title){
	printf("%s", update_styles("[808080]", 0));
	...
	if(strcmp(title, "") != 0){
		printf("%s", update_styles("[F0F0F0]", 0));
		dprt(x + 2, y, " %s ", title);
	}
}
